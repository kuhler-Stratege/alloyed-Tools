...
public class UnluckEffekte {
	...
	public static void unluckItem (Item unluckItem, Entity entityIn) {
		if (entityIn instanceof EntityLivingBase) {
			...
			PotionEffect unluckEffect = new PotionEffect(2, 300, 1, false);
	...
	public static void unluckBlock (Block walkBlock, World worldIn, Entity entityIn, int x, int y, int z) {
		if (entityIn instanceof EntityLivingBase) {
				if (worldIn.getBlock(x, y, z) == walkBlock) {
					//((EntityLivingBase)entityIn).addChatMessage(new ChatComponentText("Block gegeben false"));
					((EntityLivingBase)entityIn).addPotionEffect(new PotionEffect(2, 300, 1, false));
			}
		}
	}
}